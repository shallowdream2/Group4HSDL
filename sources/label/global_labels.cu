...
__host__ void hop_constrained_case_info::init(int n) {
  ...
  new (mmpool_labels) mmpool<hub_type>(n * 10, 100); // 调用构造函数

  cudaMallocManaged(
      &L_cuda, n * sizeof(cuda_vector<hub_type> *)); // 分配n个cuda_vector指针
  for (int i = 0; i < n; i++) {

    cudaMallocManaged(
        &L_cuda[i],
        sizeof(cuda_vector<hub_type>)); // 为每个指针分配一个cuda_vector
    new (L_cuda[i]) cuda_vector<hub_type>(mmpool_labels); // 调用构造函数
  }
