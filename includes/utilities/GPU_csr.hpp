...
#include <utilities/CPU_adj_list.hpp>
#include <vector>
/*for GPU*/
template <typename weight_type> class CSR_graph {
  // CSR has space efficiency and is easy to use by GPUs.
public:
  CSR_graph() {}
  ~CSR_graph();
  std::vector<int> INs_Neighbor_start_pointers, OUTs_Neighbor_start_pointers,
      ALL_start_pointers; // Neighbor_start_pointers[i] is the start point of
                          // neighbor information of vertex i in Edges and
                          // Edge_weights
  /*
  ...
  */
  std::vector<int> INs_Edges, OUTs_Edges,
      all_Edges; // Edges[Neighbor_start_pointers[i]] is the start of
...
CSR_graph<weight_type> toCSR(graph_structure<weight_type> &graph) {
  ...
  int V = graph.size();
  ARRAY.INs_Neighbor_start_pointers.resize(
      V + 1); // Neighbor_start_pointers[V] = Edges.size() = Edge_weights.size()
  ...
  cudaMemcpy(ARRAY.in_pointer, ARRAY.INs_Neighbor_start_pointers.data(),
             (V + 1) * sizeof(int), cudaMemcpyHostToDevice);
  cudaMemcpy(ARRAY.out_pointer, ARRAY.OUTs_Neighbor_start_pointers.data(),
             (V + 1) * sizeof(int), cudaMemcpyHostToDevice);
  cudaMemcpy(ARRAY.all_pointer, ARRAY.ALL_start_pointers.data(),
             (V + 1) * sizeof(int), cudaMemcpyHostToDevice);
  cudaMemcpy(ARRAY.in_edge, ARRAY.INs_Edges.data(), E_in * sizeof(int),
             cudaMemcpyHostToDevice);
  cudaMemcpy(ARRAY.out_edge, ARRAY.OUTs_Edges.data(), E_out * sizeof(int),
             cudaMemcpyHostToDevice);
  cudaMemcpy(ARRAY.all_edge, ARRAY.all_Edges.data(), E_all * sizeof(int),
             cudaMemcpyHostToDevice);
  cudaMemcpy(ARRAY.in_edge_weight, ARRAY.INs_Edge_weights.data(),
             E_in * sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpy(ARRAY.out_edge_weight, ARRAY.OUTs_Edge_weights.data(),
             E_out * sizeof(double), cudaMemcpyHostToDevice);
...
template <typename weight_type> CSR_graph<weight_type>::~CSR_graph() {
  cudaFree(in_pointer);
