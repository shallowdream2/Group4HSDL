...
the time complexity of enumerating k out of n: O(n^k) (imagine n for loops)

for_each_permutation() is for ordered permutations;
...
public:
    ...
    bool operator()(It first, It last)  // called for each permutation
    {
        ...
        cout << "permutation elements: "; // from first to last
        ...
        cout << "out of permutation elements: "; // from last to std::next(last, len - k), where k is in (k out of n)
        for (auto it = last; it != std::next(last, len - k); it++) {
            cout << *it;
            auto it2 = it;
            it2++;
            if (it2 != std::next(last, len - k)) {
                cout << ", ";
            }
        }
        cout << endl;
