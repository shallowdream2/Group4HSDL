...
struct two_hop_label {
  int vertex;
  int parent_vertex;
  int distance;
  two_hop_label() = default;
  ~two_hop_label() = default;
  bool operator<(two_hop_label const &y) const {
...
public:
  ...
  long long int compute_label_bit_size() {
    ...
    for (auto it = L.begin(); it != L.end(); it++) {
      size = size +
             (*it).size() * sizeof(two_hop_label); // 12 bit per two_hop_label
    }
    return size;
  }

  void clear_labels() { vector<vector<two_hop_label>>().swap(L); }
...
static int extract_distance(two_hop_case_info &case_info, int source,
                            int terminal) {
  ...
  int distance = std::numeric_limits<int>::max(); // if disconnected, return
                                                  // this large value
  ...
  while (vector1_check_pointer != pointer_L_s_end &&
    ...
    if (vector1_check_pointer->vertex == vector2_check_pointer->vertex) {
      int dis =
          vector1_check_pointer->distance + vector2_check_pointer->distance;
      if (distance > dis) {
        distance = dis;
      }
...
static void extract_shortest_path(two_hop_case_info &case_info, int source,
                                  int terminal, vector<pair<int, int>> &path) {
...
static pair<int, int>
two_hop_extract_two_predecessors(vector<vector<two_hop_label>> &L, int source,
                                 int terminal) {
  ...
  int vector1_capped_v_parent = 0, vector2_capped_v_parent = 0;
  int distance = std::numeric_limits<int>::max(); // if disconnected, retun this
                                                  // large value
  bool connected = false;
  auto vector1_check_pointer = L[source].begin();
  auto vector2_check_pointer = L[terminal].begin();
