...
/*global values*/
static graph_v_of_v<int> global_ideal_graph_CT,
...
static void dfs(int &total, vector<int> &first_pos, int x, vector<vector<int>> &son,
         vector<int> &dfn) {
  total++;
  dfn[total] = x;
  first_pos[x] = total;
...
static void CT(graph_v_of_v<int> &input_graph, CT_case_info &case_info) {
...
static void CT_cores(graph_v_of_v<int> &input_graph, CT_case_info &case_info) {
  auto begin1 = std::chrono::high_resolution_clock::now();
